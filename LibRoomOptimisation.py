...
class Constraint() :
    ...
    def GetPPCatSelf(self, persoData) : 
        """
        Define the amount of happyness generated by own user property
        """
        
        suffix = self.GetColumnsOption(persoData) if self.multi else ['']
        
        Pp = np.zeros(len(persoData.index) )
        for x in suffix :
            filterSelfLiking = ( persoData[self.inLabel+str(x)] == persoData[self.label+str(x)])
            Pp += ( filterSelfLiking * persoData[self.weightLabel+str(x)])
        return Pp
...
class TestConstraint( unittest.TestCase ):
    ...
    def test_DefinePRCatConstraint_resultInfeas(self) :
        ...
        #
        # ============================================================================= 
    def test_GetPPCatSelf_result(self) : 
         cons = Constraint( 'ppCat', 'service', True, roomTag=['etage'], removeSelf=True )
         self.assertTrue(np.allclose([3,0, 0], cons.GetPPCatSelf(self.persoData) , rtol=1e-05, atol=1e-08))
        
    def test_GetPPCatSelf_resultMulti(self) : 
        self.persoData['perso1'] = ['Dum2', 'Dum1', 'Dum2']
        self.persoData.loc[1, 'perso0'] = 'Dum1'

        cons = Constraint( 'ppCat', 'perso', True, roomTag=['etage'], removeSelf=True, multi=True )
        self.assertTrue(np.allclose([0, 8, 5], cons.GetPPCatSelf(self.persoData) , rtol=1e-05, atol=1e-08))
    ...
    def test_DefinePPCatConstraint_resultInputRemoveSelf(self) :
        ...
        self.assertTrue(np.allclose( [[0, 6], [0, 0]], cons.wish, rtol=1e-05, atol=1e-08))
        self.assertTrue(np.allclose( [[1, 0], [1, 1]], cons.dispo, rtol=1e-05, atol=1e-08))

        self.assertAlmostEqual(0, cons.GetObjVal() )
        hap = cons.GetPPHappyness(self.placement, self.officeData, self.persoData )
        self.assertTrue(np.allclose([0,0, 0], hap , rtol=1e-05, atol=1e-08))
    ...
    def test_DefinePPCatConstraint_resultMultiRemoveSelf(self) :
        ...
        self.assertTrue(np.allclose( [[0, 8], [3, 0], [1, 0]], cons.wish, rtol=1e-05, atol=1e-08))
        self.assertTrue(np.allclose( [[1, 0], [1, 0], [0, 1]], cons.dispo, rtol=1e-05, atol=1e-08))
